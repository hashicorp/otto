...
func creds(ctx *infrastructure.Context) (map[string]string, error) {
	fields := []*ui.InputOpts{
		&ui.InputOpts{
			...
			Description: "Token used for API calls.",
			EnvVars:     []string{"DO_TOKEN"},
		},
		&ui.InputOpts{
			Id:          "ssh_public_key_path",
			Query:       "SSH Public Key Path",
			Description: "Path to an SSH public key that will be granted access to the Droplets",
			Default:     "~/.ssh/id_rsa.pub",
			EnvVars:     []string{"SSH_PUBLIC_KEY_PATH"},
		},
	}
